...
local function setup_keymaps()
    ...
    map("n", "<leader>mm", function() make_qf() end,
        vim.tbl_extend("force", opts, { desc = "Build" }))
    map("n", "<leader>ms", select_target,
        vim.tbl_extend("force", opts, { desc = "Select Target" }))
    map("n", "<F5>", function() make_qf() end,
        vim.tbl_extend("force", opts, { desc = "Build" }))
    ...
    for _, config in ipairs(TARGET_CONFIGS) do
        if targets[config.name] then
            ...
            local func = config.func_type == "sync" 
                and function() make_sync(config.name) end
                or function() make_qf(config.name) end
